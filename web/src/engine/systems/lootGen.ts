...
/** Smith-forged monster trinket: a second accessory line, 2-3 affixes. */
export function forgeTrinket(ilvl: number, luck = 0): ItemV2 {
  const level = Math.max(1, ilvl);
  const material = materialForIlvl('Trinket', level);
  const affixes = rollAffixes(level, 2 + (randInt(100) < 25 + luck ? 1 : 0));
  const rarity: ItemRarity = affixes.length >= 3 ? 'Rare' : 'Magic';
  return {
    uid: freshUid('item'),
    baseType: 'Trinket',
    ...
    ilvl: level,
    rarity,
    name: buildName(`${material} Trinket`, rarity, affixes),
    implicitAttack: 0,
    implicitMagic: 0,
    implicitDefense: 0,
    affixes,
    value: computeValue('Trinket', material, level, rarity, affixes.length),
  };
}
...
export function generateItem(ilvl: number, luck = 0, bias = 0): ItemV2 {
  ...
  const typeNames = (Object.keys(ITEM_TYPES) as ItemTypeName[]).filter((t) => t !== 'Charm' && t !== 'Trinket');
