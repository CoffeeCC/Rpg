 ...
 *
 * AND THEN IT PUBLISHES, which is the other half of that decision and was
 * missing until now. Staging is unreachable from a build: `lantern-board.html`
 * reads it through Vite's `/@fs/` escape hatch, which is a DEV SERVER feature
 * pointed at an absolute path on one developer's disk. Nothing that ships can
 * use it, so the renderer on the game path was loading the ghostly original
 * albedo and no relief at all while the harness next to it looked right.
 *
 * So every write is followed by a copy of the two maps the RENDERER SAMPLES —
 * `_normal.png` and `_albedo.png` — into `web/public/art/materials/<set>/`.
 * Height and AO stay behind, because they are inputs to the normal and nothing
 * at runtime reads them; publishing all four would double the payload to no
 * effect. The published tree is a build artifact in every sense — gitignored,
 * derived, and regenerated byte-for-byte by this tool — it just happens to
 * live where Vite can see it. `--no-publish` skips the copy.
 *
 * It is a BYTE COPY and deliberately not a re-encode or a downsample. The
 * published file is provably the staged file, so "does the game see the map I
 * baked" is a checksum rather than an argument. The payload that buys is
 * printed in the summary rather than left to be discovered.
 *
 * AND THAT PAYLOAD IS THE OPEN QUESTION HERE. The whole published tree is
 * ~54 MB, of which the tiles are ~26 MB — a normal map is an incompressible
 * 1024x1024 noise field and PNG cannot help it. A monster costs about 0.7 MB
 * for both maps and a floor loads a handful; a GATE costs 5.4 MB for its two
 * tile normals and a floor loads both of them before the ground looks like
 * stone. Measured worth it (without them the floor is laminate under a raking
 * lantern, with them it has grain) and measured expensive.
 *
 * The obvious next move is to halve the tile normals to 512 on publish and
 * renormalise, which is roughly a quarter of the bytes for a map sampled at
 * 40-90 screen pixels per tile. Deliberately NOT done here: a resample that
 * forgets to renormalise is a lighting bug that looks like art, and this
 * change is already the difference between "the renderer cannot see the bakes
 * at all" and "it can". One thing at a time.
 */

import { readdirSync, existsSync, readFileSync, writeFileSync, mkdirSync, statSync, copyFileSync } from 'node:fs';
...
/** Where the renderer looks. Must match `BAKED_ROOT` in `web/src/render/materials.ts`. */
const PUBLISH_ROOT = join(ROOT, 'web', 'public', 'art', 'materials');
/** The maps the shader samples. Height and AO are inputs, not deliverables. */
const PUBLISHED_MAPS = ['normal', 'albedo'];
...
material baker — bakes normal/height/AO off source art, headless
  ...
  --no-publish      bake into staging only; do not copy the normal and albedo
                    maps into web/public/art/materials/, which is where the
                    renderer looks and the only place a real build can reach
...
/**
 * Copy one asset's runtime maps out of staging and into `public/`.
 *
 * Takes the manifest ROW rather than a path, so the published set is exactly
 * the baked set by construction: there is no second list of assets to fall out
 * of step, and an asset whose bake produced no albedo (every tile — luminance
 * ->Sobel has no alpha to repair) simply has nothing to copy for that map.
 *
 * Returns the bytes published, because that is the cost of this decision and a
 * cost nobody sees is a cost nobody manages.
 */
export function publishOne(row) {
  let bytes = 0;
  for (const map of PUBLISHED_MAPS) {
    const rel = row.outputs[map];
    if (!rel) continue;
    const from = join(ROOT, rel);
    if (!existsSync(from)) continue;
    const to = join(PUBLISH_ROOT, row.set, basename(rel));
...
export function main(argv) {
  ...
  for (const job of jobs) {
    ...
    rows.push(row);
    // Publish only what was actually written. `--dry` measures; it must not
    // leave a public/ tree describing a bake that was never performed.
    if (opts.write && opts.publish) published += publishOne(row);
  ...
  else if (!opts.quiet) process.stdout.write(summary(rows, Date.now() - t0, opts.write && opts.publish ? published : null) + '\n');
...
function summary(rows, ms, publishedBytes) {
  ...
  // What the renderer can now reach, and what it weighs. Printed rather than
  // silent because this is the one output of the tool that ends up in `dist`,
  // and a tile normal is an incompressible 1024x1024 field — the largest
  // single thing this pipeline produces.
  if (publishedBytes !== null && publishedBytes !== undefined) {
    lines.push(
      `  publish  ${PUBLISHED_MAPS.join(' + ')} -> web/public/art/materials/ · ${(publishedBytes / 1048576).toFixed(1)} MB this run`,
    );
  }
