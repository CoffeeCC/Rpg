...
## 23. The console deck stops being a flat colour (2026-07-31)

`docs/ART_QC_FURNITURE_2026-07-26.md` shipped a QC'd sample the day before —
painted albedo for `console_body`/`console_body_brass`, cut into the exact
shipped silhouette, alpha byte-identical, normal/material maps untouched — and
left it staged with `"installed": false`, explicitly for Paul to judge before
committing to the rest of the set. Judged on sight against the §7 de-shading
brief (flat even light, no baked shadow, seamless, PNG) by the orchestrator on
this pass: the timber reads as real oiled oak with knots and scratches, the
round-2 brass reads as brushed metal rather than the round-1 "crushed pepper"
the QC doc itself rejected, and the 2x2 tile sheet shows no seam. Installed —
this one piece only.

**What actually moved.** `web/public/art/materials/board/console_body.png` and
`console_body_brass.png` are gitignored build artifacts (`web/.gitignore`'s
"derived, regenerated byte-for-byte" note), so "installing" means the two
staged PNGs are copied over them, byte-for-byte, nothing else. No code
changed — `battleScene.ts`'s deck placement, UV repeat and layer were already
wired by `1fff3ad`; only the pixels the two texture ids resolve to are
different now. That is also why this needed no new test: the aspect-ratio
test (§22 above) is unchanged (both files stayed 1024x348), and nothing
asserts a colour on that path.

**Made durable.** `web/art-staging/` is gitignored working storage, so the two
reusable source materials (`console_timber.png`, `console_brass.png`, both
1024x1024 seamless), the two cut albedos actually installed, and the brass
generator (`_gen_console_brass.py` — round 2 was procedural, not a second Grok
call) are now tracked at `tools/art/furniture-materials/`, next to a
`manifest.json` recording what replaces what and the measured before/after.
Without this they would not survive an `art-staging` cleanup, and the QC
doc's own "the timber and brass materials here are reusable for all of them
as-is" claim would have nothing to point at.

**Verified:** alpha channels of both new files confirmed byte-identical to
the files they replaced before the swap (`np.array_equal`, both true); `tsc -b`
clean; `npx vitest run` from `web/` green (1465 passed, 3 skipped, 60 files).
**Not verified by a composited frame** — the browser pane did not composite in
this environment, consistent with §22's own finding, and reaching a live
battle needs a run in progress rather than a standalone debug page the way
the map has `lantern-board.html`. Verification here is by direct inspection
of the final PNGs (opened and looked at, not trusted from a filename) plus
the measurements above, which is a narrower bar than §22's but matches the
size of the change: a texture-content swap on a render path that was already
wired, tested, and unchanged in every other respect.

**Still open, unchanged from §22.5:** `log_well`, `pile_tray`,
`lantern_cradle`, `brass_strap`, `exhaust_grate` and the four wall shapes are
still flat-fill. The QC doc's source materials are reusable for the wood and
brass among them; stone and vellum (whatever `log_well`'s parchment ends up
being) still need their own commission. This is a real art bill, not a code
task, so it is the next thing to ask Grok for rather than the next thing to
build.
